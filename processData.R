...
processData <- function(directory) {
  ...
  testClin <- clin[,c("Tumor_Sample_Barcode","UV_sig","UV_sig_value","NER_sig")]
  testClin <- testClin[c(50,286,329),]
  testMut <- mutations[mutations$Tumor_Sample_Barcode %in% testClin$Tumor_Sample_Barcode, ]
  testMut <- testMut[testMut$Hugo_Symbol %in% NERgenes, ]
